...
- [x] T093 [US5] Extender `BenchmarkPlan` y validación bilateral para dirección única, simultáneo, UDP y capacidad/tasa manual en `src-tauri/src/control/plan.rs`; límites fijos según FR-042c (antes de partirse, FR-042): 1–64 streams en secuencial y 1–32 por sentido en simultáneo; V-01 (T100) valida la viabilidad del motor y los puertos, no cambia el límite
...
- [x] T125 [US7] Registrar en `docs/governance/ADRS.md` y `specs/001-network-benchmark-v1/checklists/architecture.md` la forma acordada del updater (FR-042d, antes de partirse FR-042: `latest.json` estable cuyo artefacto apunta a `releases/download/vX.Y.Z/…`, nunca a un binario mutable) y definir la prueba que la verifica antes de tocar el updater
...
- [x] T144 CRITICAL Implementar la orquestación real de sesión en `src-tauri/src/control/service.rs`: preflight, negociación de plan, ejecución de NTTTCP, muestreo, diagnóstico, persistencia y limpieza; hoy `SessionService` solo transiciona estados y los módulos `engine::ntttcp`, `sampling`, `preflight`, `pairing`, `diagnostic::rules` y `control::cleanup` no se invocan desde ningún camino de ejecución, según FR-018–FR-023 y US1/AC1 (missing) — **parcial el 2026-09-23**: `control/engine_port.rs` expone el motor como puerto (`MotorDeMedida`, con `MotorNtttcp` real y `MotorDeLaboratorio` para pruebas) y `control/orquestador.rs` ejecuta la mitad local de una dirección, ensambla `DirectionResult` y `SessionResult`, aplica diagnóstico y toma `officialBps` solo del receptor. `app::init` construye el orquestador con el motor real. Falta el diálogo con el peer (PREPARE/READY/START, T152), el muestreo en vivo durante la ejecución y la persistencia automática al cierre de sesión — **avance sustancial el 2026-09-24**: `control/session_flow.rs` implementa el diálogo completo REQUEST/RESPONSE, PREPARE/READY, negociación de inicio y ENGINE_DONE para una dirección TCP secuencial, y `tests/session_flow_real.rs` lo prueba de punta a punta con NTTTCP real entre dos `ControlServer` reales: 5,28 s, resultado `completed` con la misma velocidad oficial en los dos extremos (FR-025, FR-027). **Sigue sin cablearse a `ipc::session::session_start`**: ese comando aún solo transiciona la máquina de estados sin conectar con nada. Faltan también: bidireccional en una misma sesión, cancelación a mitad de diálogo (CANCEL/CANCEL_ACK), persistencia automática y muestreo en vivo (T150 construyó el mecanismo de emisión, sin bucle que lo alimente) — **cableado el 2026-09-25**: `session_start` ya lanza la sesión real (`SessionService::iniciar_sesion_real`) y el bucle del servidor despacha las solicitudes entrantes (`control/despachador.rs`). Ida y vuelta en una sesión, persistencia automática en `history` de los dos extremos con el mismo `session_id` y las mismas cifras, cancelación local por aborto de tarea (<2 s, sin `ntttcp.exe` huérfano), rechazo por desconocido/sin autoaceptación/ocupado, y comprobación de identidad al conectar (FR-013). Verificado con NTTTCP real entre dos `SessionService` completos en bucle local. **T144 sigue abierta** por lo que la tarea nombraba y no está hecho: preflight en el recorrido de sesión (FR-019/FR-020), cancelación graciosa entre peers, intercambio `SESSION_RESULT`/`SESSION_ACK`, y consentimiento local. El muestreo en vivo se cerró aparte el 2026-09-25 (T174). Se desglosan en la Fase 13 — **cerrada el 2026-09-25**: los cuatro huecos que declaraba abiertos se cerraron por su cuenta en la Fase 13 desglosada, y se han comprobado uno por uno antes de marcar esto como hecho, en vez de dar por buena la nota de otra tarea: preflight en el recorrido de sesión (T173, `PreflightEvaluator::check_nic`/`check_ports` invocados de verdad en `session_flow.rs`, no solo probados) · cancelación graciosa `CANCEL`/`CANCEL_ACK` (T175) · intercambio `SESSION_RESULT`/`SESSION_ACK` y reconciliación (T176) · consentimiento local, con su pantalla (T177 + T181 + T182, emparejamiento entrante incluido). Bidireccional secuencial en una misma sesión, persistencia automática en los dos extremos y limpieza sin `ntttcp.exe` huérfano: verificados con NTTTCP real entre dos `SessionService` completos (cerrado el 2026-09-25, nota anterior de esta misma tarea). **Un hallazgo al revisar la limpieza (FR-023) antes de cerrar**: la propia tarea nombraba `control::cleanup` como módulo a invocar; existe (`CleanupCoordinator`), tiene su propio test (`process_cleanup.rs`) y pasa, pero **no lo invoca nadie fuera de su fichero** — verificado por grep, cero usos en `AppState` ni en `session_flow.rs`/`service.rs`. La limpieza real que sí funciona y que los tests de NTTTCP real comprueban es RAII: `Drop` de `JobObject` y `NtttcpProcess` matan el proceso y liberan el job al soltarse. La cita de `control/cleanup.rs` como evidencia de FR-023 en `traceability.md` era incorrecta y queda corregida. **Decisión que corresponde al propietario, no tomada aquí**: si `CleanupCoordinator` se cablea de verdad o se retira por no usarse — no se ha borrado ni modificado, solo se ha dejado de citar como si estuviera integrado. **Fuera del alcance de esta tarea, con sus propias entradas**: ejecución simultánea real (`BenchmarkDirection::Both`, T178/T183), verificación entre dos equipos físicos (T180), arnés de rendimiento (T159)
...
- [x] T147 CRITICAL Sustituir las suites `expect(true).toBe(true)` de `e2e/smoke.spec.ts`, `e2e/accessibility/a11y.spec.ts` y `e2e/visual/visual.spec.ts` por comprobaciones reales con axe-core, añadir el script `test:e2e` y enlazarlo a `scripts/verify-app.mjs` o al checkpoint que corresponda, según SC-008 y FR-039/FR-041 (contradicts) — **hecha el 2026-09-25, con 4 defectos reales encontrados y corregidos**: las tres suites eran `expect(true).toBe(true)`; ahora las 9 pruebas reales pasan en verde contra un build de producción (~25 s), con 1 `test.skip` declarado. `e2e/axe.ts` inyecta el bundle de `axe-core` (ya era devDependency sin uso; no se añadió `@axe-core/playwright` sin autorización) con `page.addScriptTag` y ejecuta `axe.run()`. **El hallazgo más grave: `src/lib/design-system/tokens.css` (524 líneas, 888 usos de `var(--color-*)` en 46 componentes) no se importaba en ningún sitio** — `src/app.css` solo tenía `@import "tailwindcss"`. Sin el `@import` añadido, la aplicación entera se habría visto sin ningún color de marca en un WebView2 real; lo detectó el primer test de `visual.spec.ts` y se confirmó con una captura real (tema oscuro «Graphite Violet» renderizando correctamente tras el arreglo). **Dos defectos de accesibilidad reales**: el `tablist` de `SettingsScreen` y el `radiogroup` de tema de `AppearanceSettings` declaraban su rol ARIA compuesto sin implementar el patrón de teclado APG (sin flechas, sin roving `tabindex`) — corregido en los dos; y axe-core marcó el texto de `TitleBar.svelte` como fuera de cualquier landmark (RGAA-9.2.1) — su raíz era un `<div>`, ahora es `<header>` (banner implícito). **Un defecto de integración que el propio `Dialog.svelte` ya advertía en su comentario**: marcaba `inert` solo un nivel de ancestros, válido solo si el diálogo cuelga justo del contenedor de ruta; en la integración real (T181) el diálogo de conexión manual queda varios niveles por debajo de la barra lateral, que **no** quedaba inert — corregido para subir por toda la cadena hasta `<body>`. **Un cuarto, en el mismo diálogo**: el retorno de foco al cerrar es responsabilidad documentada de quien instancia el diálogo; `PeersScreen.svelte` nunca lo implementaba — corregido con `document.activeElement` capturado al abrir y `await tick()` antes de restaurar (sin el `tick()`, el botón seguía `inert` por el `onDestroy` del diálogo, que Svelte no aplica de forma síncrona). **Obstáculo resuelto, no solo declarado**: las primeras ejecuciones contra `vite dev` colgaban de forma intermitente por compilación bajo demanda en frío (aislado con `curl -m 90` y `workers: 1`); arreglado sirviendo el build de producción (`vite build && vite preview`) en `playwright.config.ts`, sin esa fuente de intermitencia. `tsconfig.json` ganó `e2e/**/*.ts` y `playwright.config.ts` en `include` (491 ficheros, 0 errores). `package.json` gana `test:e2e`. `.gitignore`/`.prettierignore` ganan `test-results/`, `playwright-report/`, `.playwright-mcp/`. **`scripts/verify-app.mjs` NO encadena `test:e2e`, a propósito** (decisión declarada): un E2E necesita un build previo y un navegador real, con más superficie de fallo ajena al código que el resto del gate; queda como comando manual, con la razón impresa al final de `verify-app.mjs`. **No hecho**: el tercer test visual («plantilla de impresión PDF A4») se dejó `test.skip` con motivo — `PrintReport.svelte` no tiene ninguna ruta, ventana ni regla `@media print` que lo haga alcanzable (verificado por grep en todo `src/`); corresponde a quien cablee la exportación a PDF, fuera de esta tarea. El zoom 100/150/200% del test visual es una aproximación con zoom CSS, no el escalado real de DPI de Windows, que este entorno no puede fijar
...
- [ ] T150 Emitir eventos Tauri de progreso, muestras y cambios de estado desde `src-tauri/src/ipc/` y `src-tauri/src/sampling/`; hoy no hay ninguna llamada a `.emit(` en Rust mientras `src/lib/api/samples.ts` y `src/lib/api/transport.ts` escuchan, según FR-026 y FR-044 (missing) — **parcial el 2026-09-24**: `ipc/events.rs` implementa `EmisorDeEventos` para `session://sample-batch` (el único evento con contrato real en el frontend), probado con doble de prueba en Rust y con `tests/contracts/samples.test.ts` verificando la forma exacta en los dos lados. **No cableado a ningún camino de ejecución real**: no hay bucle de muestreo en vivo durante una sesión (depende de T144), y «progreso» y «cambios de estado» no tienen contrato de evento definido en el frontend — hoy ese papel lo cumple la suscripción con `revision` de `ipc/snapshot.rs` (T024). Inventar un evento sin consumidor habría sido una decisión de producto, no una corrección — **cambios de estado cableados el 2026-09-25**: el snapshot se construía una vez al arrancar y `SnapshotManager::update` no se invocaba desde ningún sitio, así que `isSessionActive`, `activeSessionId`, `peersCount`, idioma y tema quedaban congelados —y `App.svelte` decide con `isSessionActive` el fondo y el botón, por lo que la interfaz nunca habría visto una sesión en marcha—. Ahora `SessionStateMachine` avisa con un `Notify` en cada transición, `ipc/proyector.rs::ProyectorDeSnapshot` **recalcula desde las fuentes** (ajustes, máquina de estados, tabla `peers`) y `SnapshotManager::sincronizar` solo sube la revisión si algo cambió; un cambio se emite como `app://snapshot-changed` con el snapshot completo. También avisan `settings_update`, `peers_manual_connect`, `peers_pairing_confirm` y el emparejamiento entrante, y `app_get_snapshot` recalcula antes de responder. En el frontend `snapshotStore` se suscribe **antes** de pedir el snapshot (`contracts/ipc.md`), descarta eventos con `revision <=` la actual y sustituye por el snapshot entrante; el JSON del evento es el del fixture `app-snapshot` (T160). Verificado con 3 pruebas del proyector (sesión que empieza y termina, aviso sin cambios no emite, contador de equipos), 2 de `sincronizar` y 5 de `aplicarEvento`. **Decisión declarada (INFERIDA)**: `contracts/ipc.md` lista `session.changed` y `app.snapshotInvalidated`, no un evento de snapshot completo; se eligió el snapshot completo porque es autosuficiente (no exige reconstruir por partes ni detectar huecos) y porque `ipc.md` ya lo describe como modelo de lectura; queda anotado como fila nueva en la tabla de eventos. **Sigue abierta** por: (1) no hay evento de **progreso** de sesión ni de comprobaciones (`session.changed` con `checks`), ni consumidor en la interfaz; (2) no se ha probado con un `AppHandle` real (el emisor de Tauri se ejercita solo con dobles; sin WebView2 real no se comprueba que el evento llegue); (3) `peersCount` cuenta la tabla `peers` (equipos conocidos), no los descubiertos por mDNS, y la eliminación de equipos no avisa porque no existe comando que los borre; (4) el snapshot no incluye aún la proyección de sesión/solicitud actual ni los equipos que `ipc.md` menciona. **Dos defectos hermanos corregidos de paso**: `settings_update` y `app_close_evaluate` trataban `Completed` y `Failed` como sesión activa (`!= Idle && != Cancelled`), así que tras una sesión terminada se rechazaban los cambios de ajustes y cerrar la aplicación pedía confirmación como si midiera; ahora usan `SessionState::is_active()`, sin prueba automatizada propia (requieren un `tauri::State`)
- [ ] T151 Implementar el anuncio y el descubrimiento mDNS con `mdns-sd` en `src-tauri/src/discovery/` (FR-010, `Historias.md` §7.1) — **backend hecho el 2026-09-25, sin interfaz ni verificación entre equipos distintos**: nuevo `discovery/anuncio.rs`. Publica `_networkbench._tcp.local.` con nombre de instancia `<displayName> [<8 hex>]` y los registros TXT `v`/`app`/`id`/`fp`/`name`/`link`/`busy` de §7.1, con direcciones automáticas por interfaz, y navega por los demás; un equipo que se cierra retira su anuncio (`Drop`). Se corrige la constante `MDNS_SERVICE_TYPE`, que decía `_netbench` y no coincidía con la especificación —habría hecho invisible a la aplicación para cualquier otra implementación—. **Lo anunciado no es confiable** (FR-011): `equipo_desde_anuncio` es una función pura y estricta que valida cada campo (UUID, huella de 64 hex, nombre saneado con el mismo `sanitize_display_name` de la conexión manual, versión y `link` acotados, puerto ≥ 1024, descarta direcciones inalcanzables —sin especificar, multidifusión, bucle local, IPv6 de enlace local—) y rechaza el anuncio entero ante cualquier duda; el resultado (`EquipoDescubierto.fingerprint_declarada`) es una pista, no una identidad, y no lleva ningún estado de confianza. La instancia no aparece en su propia lista. Cableado: `AppState::aplicar_descubrimiento` lo enciende/apaga según el ajuste `mdns_enabled` (por defecto encendido) al arrancar y **al instante** al cambiar el ajuste (`settings_update`), y `peers_discovered_list` (IPC) lo expone. Verificado con 7 pruebas puras del análisis de anuncios (incluidos anuncios hostiles e incompletos) y con **multicast real**: `tests/mdns_real.rs` (`--ignored`) — dos instancias en esta máquina se ven mutuamente con nombre, huella declarada y puerto correctos, ninguna se ve a sí misma, y una desaparece de la lista de la otra al cerrarse (2,3 s). **Pendiente, y por eso sigue abierta**: (1) no existe la pantalla «Equipos disponibles» que consuma `peers_discovered_list` (el frontend no lo llama); (2) solo se ha probado entre dos instancias de **la misma máquina**, no entre equipos distintos, ni con varios adaptadores ni con VPN, ni con el perfil de firewall Público (V-08); (3) «una entrada desaparece 30 s después del último anuncio» se delega en la caducidad de registros de `mdns-sd`, sin medir; (4) faltan el aviso «sin equipos tras 5 s en perfil público» y el estado «Versión incompatible»/«Ocupado» mostrado (el campo `busy` se anuncia siempre a `0`, no refleja la sesión activa) y `link` siempre `0`; (5) el puerto anunciado es el de control por defecto (`CONTROL_PORT_DEFAULT`), no uno configurable; (6) el cableado a `AppState` y a `settings_update` no tiene prueba propia (construir un `AppState` real tocaría `%LOCALAPPDATA%`)
...
- [ ] T154 Rediseñar la elevación del helper en `src-tauri/src/firewall/helper_client.rs` (FR-057, plan §Plan de seguridad·2/6) — **implementada el 2026-09-25, la elevación real sin verificar**: (1) ya no hay `powershell -Command` ni cadenas interpoladas: `ShellExecuteExW` con `runas` directo (`helper_client::lanzar_elevado`). (2) ya no hay fichero de petición en `%TEMP%`: la petición son los argumentos con los que nace el proceso elevado (`--rule <op> "<nombre>" <proto> <puertos> "<programa>" <perfiles>`), sin nada que sustituir entre escritura y lectura; el fichero de respuesta, que nadie leía, también desaparece y la respuesta es el código de salida. (3) ya no se resuelve el helper por `PATH`: `get_helper_path` devuelve `Option` en vez de recurrir al nombre suelto, que `runas` habría resuelto por el directorio actual o el `PATH`; y el helper llama a `netsh` por ruta absoluta de `System32`. (4) **la lista blanca tenía tres agujeros que la tarea no nombraba** y se cerraron en un módulo único, `firewall/validation.rs`, compilado tanto en la biblioteca como en el helper con `#[path]` para que no puedan divergir (estaba duplicada): el programa se comprobaba por el sufijo del nombre —`C:\\malo\\ntttcp.exe` pasaba— y ahora exige nombre exacto **y** carpeta exacta de la instalación, canonicalizada; el rango de puertos solo se comprobaba con exactamente un guion —`1024-1030,2000-65000` se saltaba el límite de 64— y ahora solo admite un puerto o `a-b` de ≤ 64, ≥ 1024, sin listas; y protocolo y perfiles no se comprobaban en absoluto. Además una regla `add` sin programa ya no se aplica (el helper la creaba sin acotar a programa si el fichero no existía). Verificado con 10 pruebas de la lista blanca en `validation.rs` y 5 pruebas que **ejecutan el binario real del helper** con `--validate-only` (sin privilegios, sin tocar el firewall) pasándole la línea de parámetros exacta del cliente, con una carpeta con espacios: una petición válida llega intacta, y el helper rechaza por su cuenta un `ntttcp.exe` de otra carpeta, listas de rangos, todos los puertos, protocolo y perfil inventados, reglas ajenas, operación desconocida y un programa ajeno dentro de su carpeta, y no ignora argumentos que no conoce (incluido el `--file` del protocolo antiguo). **NO VERIFICABLE aquí**: la llamada a `ShellExecuteExW` con `runas` en sí —abre un diálogo de UAC en el escritorio del usuario y una prueba automática no debe aceptarlo—; nadie la ha ejecutado, y el mapeo de «UAC rechazado» a `FirewallUacRejected` tampoco. **Desviaciones declaradas de `Historias.md` §14.2**: el texto dice «fichero JSON» y `INetFwPolicy2` (COM, no `netsh`); aquí la petición viaja por argumentos —es justo lo que pide esta tarea: un canal no manipulable— y el helper sigue usando `netsh` con argumentos separados y validados; cambiar a COM es otra tarea. Queda abierta hasta que alguien acepte un UAC una vez y confirme que una regla se crea
...
- [ ] T159 Crear el arnés de rendimiento en `tests/windows/performance/`, hoy solo con `README.md`, y medir V-04/V-12 sobre la aplicación en ejecución; `scripts/test/performance.ps1` muestrea procesos que no existen, según T054 y SC-010 (partial) — **arnés real construido y ejecutado el 2026-09-25, medición parcial**: el script no lanzaba nada; muestreaba procesos que no existían y nunca calculaba CPU (`$totalCpu` declarada y sin usar). Reescrito: lanza `NetworkBench.exe` de verdad, recorre su árbol de procesos completo (la app + los `msedgewebview2.exe` que arranca) y mide CPU real por delta de `TotalProcessorTime` y memoria real (`WorkingSet64`), limpiando después solo esos PID. **Extendido después con el método exacto de la constitución (principio VII, T168)**: exclusión explícita de NTTTCP de la suma, documentación completa del hardware (CPU/GPU/RAM/NIC/Windows/WebView2/resolución/plan de energía), y protocolo de 5 ejecuciones por escenario con/sin animación (`-ReduceMotion`, que fija `reduceMotion` en `settings.json` antes de medir). **Ejecutado de verdad el protocolo completo con el build release**: 5/5 con animación (CPU media 0,531 %, pico 0,936 %, memoria media 354,9 MB) y 5/5 sin animación (CPU media 0,374 %, pico 0,780 %, memoria media 354,6 MB) — ambas muy por debajo del 5 % de V-04, verificado sin dejar procesos huérfanos tras las 10 ejecuciones. **Sigue abierta**: esto mide la app en reposo, no «durante la prueba» (`RUNNING_*`), que es lo que V-04 presupuesta literalmente — hace falta un segundo equipo o una instancia emparejada para ejercitar una sesión real mientras se mide. La ventana del sistema se vio con geometría degenerada (~16×16 px) en una captura manual anterior con el binario de depuración, sin investigar la causa (posible fallo de detección de monitores en este entorno). Detalle en `VALIDACION.md` §1.34
- [x] T160 Añadir a `scripts/architecture/` los checks de contratos y de APIs legacy que T011 exige e integrarlos en `scripts/verify-app.mjs`, que hoy solo invoca imports, locales y logger, según T011 (partial) — **cerrada el 2026-09-25**. (1) **Contratos**: hasta ahora los esquemas Zod se probaban solo con JSON escrito a mano y nadie parseaba lo que Rust realmente serializa. `src-tauri/tests/contract_fixtures.rs` construye con el código real (orquestador con motor de laboratorio, `Peer`, `AppSnapshot`, `SampleBatcher`, `AppError`) 8 documentos (`session-result-tcp/udp`, `plan-tcp/udp`, `peers` en los 4 estados de confianza, `app-snapshot`, `sample-batch`, `app-errors`) y los compara byte a byte con `tests/contracts/fixtures/rust/` (se regeneran con `NB_UPDATE_FIXTURES=1`); `tests/contracts/rust-fixtures.test.ts` parsea esos mismos ficheros con los esquemas Zod del frontend. Resultado: **el JSON de Rust cumple hoy los esquemas TS**, sin desajuste real (los únicos fallos fueron UUID no v4 de mis propios fixtures, corregidos). (2) **APIs legacy**: `scripts/architecture/check-legacy.mjs` prohíbe `export let`, `$:`, `createEventDispatcher`, `<slot>`, SvelteKit (`$app/`, `$env/`, `+page`, `+layout`), `process.env` e `import.meta.env` fuera de `lib/config` (CONVENTIONS.md), ignorando comentarios; se probó con violaciones temporales. Encontró un defecto real: `lib/logging` leía `import.meta.env` con un cast sin validar y saltándose `lib/config`, que nadie usaba; ahora el logger usa `clientConfig` y se conserva la precedencia (`VITE_LOG_LEVEL` > debug en desarrollo / warn en producción; se quitó el `default("info")` de la config, que la habría alterado). Ambos checks están en `verify-app.mjs`, y los fixtures generados se excluyen de Prettier (`.prettierignore`) porque se comparan byte a byte. **No cubre**: `any` evasivo, `!` ni dobles casts (patrones que un regex no distingue de código legítimo; los cubre ESLint donde esté configurado, sin comprobar aquí regla por regla); el check de contratos ata la forma del JSON, no su semántica; los eventos y comandos IPC no están todos en fixtures
...
- [x] T168 Definir el «núcleo de referencia» de SC-010 («menos del 5 % de un núcleo de referencia»): hoy no está definido en ningún artefacto, así que el criterio no es evaluable. Fijar CPU, frecuencia y condiciones al cerrar V-04 y registrarlo en `VALIDACION.md`, según SC-010 (partial) — **resuelta el 2026-09-25, no hacía falta una decisión nueva**: `.specify/memory/constitution.md`, principio VII, línea 181, ya define el criterio con precisión: presupuesto «menos del 5 % de UN procesador lógico», fórmula `100 × suma(delta CPU usuario+sistema) / delta tiempo real` **sin dividir por el número de núcleos**, excluyendo NTTTCP, con hardware documentado por ejecución (CPU/GPU/RAM/NIC, Windows, WebView2, resolución/DPI, plan de energía) y cinco ejecuciones release por escenario (con/sin animación). La tarea pedía «fijar CPU, frecuencia y condiciones», pero la propia definición constitucional descarta deliberadamente fijar un modelo de CPU cerrado: normaliza a «un procesador lógico» (portable entre máquinas) y exige documentar el hardware real de cada ejecución en su lugar. `scripts/test/performance.ps1` (T159) ya sigue ese método al pie de la letra y lo ejecutó de verdad con el build release: CPU media 0,531 % con animación y 0,374 % sin animación, ambas registradas en `VALIDACION.md` §1.34 junto con el hardware completo de la máquina usada. Nada que decidir de oficio: el criterio evaluable ya existía, solo faltaba enlazarlo y aplicarlo
...
- [x] T170 **(requiere decisión del propietario: modifica requisitos)** Proponer partir FR-042, que agrupa cinco obligaciones sin relación entre sí —instalación sin red, persistencia en H1, límites de streams, forma del manifiesto de actualización y accesibilidad completa en H2— en requisitos independientes. Mientras siga fundido, la cobertura por obligación no es trazable, según FR-042 (partial) — **cerrada el 2026-09-25, con tu decisión de numeración**: partida en `spec.md` en `FR-042a` (instalación sin red), `FR-042b` (persistencia desde H1), `FR-042c` (streams 1-64/1-32), `FR-042d` (manifiesto `latest.json` estable) y `FR-042e` (accesibilidad completa en H2), con sub-letras bajo el mismo número —decidiste esto en vez de renumerar al final (FR-068-072) para no desplazar FR-043..FR-067 ni sus referencias cruzadas—. Actualizadas las 6 citas de «FR-042» que ya existían: `data-model.md`, `plan.md` (4 citas), `research.md` (2, como anotación sobre el registro histórico del conflicto, sin reescribirlo), `checklists/architecture.md` (3) y `checklists/traceability.md`, que ya tenía el desglose con evidencia real por obligación (T146, T025/T026, T093, T125/T126) desde antes de esta tarea — solo le faltaban los identificadores FR propios; se aprovechó para corregir la fila de accesibilidad (T147 ya sustituyó los stubs de axe-core que citaba como `NO PRESENTE` por pruebas reales de H1, pero H2 —Narrador, alto contraste, 200 % integral— sigue sin empezar, así que FR-042e se queda en `NO PRESENTE` con el motivo correcto). **No se ha recontado** la cifra global «de 67 FR» de la cabecera de `traceability.md`: quedó desactualizada por los cambios de todo el 2026-09-25, no solo por esta tarea, y recontarla fila por fila es su propia tarea (declarado en el propio documento en vez de inventar un número)
- [x] T171 Añadir prueba de aceptación de FR-024: «los huecos MUST marcarse y un sentido parcial MUST NOT reanudarse como continuo» — **cerrada el 2026-09-25, y la prueba encontró un fallo real**: si el canal caía en la **segunda** pata, `ejecutar_prueba_estandar_como_*` propagaba el error con `?` y **se perdía la primera pata ya completa**: la sesión terminaba `Failed` sin guardar nada, contra `Historias.md` §8.5/§19.3 («las direcciones completadas se conservan y la sesión se guarda como incompleta; nunca se reanuda una dirección a medias»). Ahora, ante una pérdida de canal o de motor tras completar una pata, la prueba devuelve lo completado, marca la otra `incomplete` sin velocidad oficial ni motores (`PruebaBidireccional.interrupcion` dice por qué), se guarda como sesión incompleta (`is_partial`) y termina `Failed` con el motivo `NB-CONN-005`; no se intenta el intercambio `SESSION_RESULT` por un canal perdido, y el respondedor conserva su vista `local`. Una **cancelación** no se confunde con una pérdida (`conservar_lo_completado`): sigue descartando la sesión. De paso, `tokio::join!` en la mitad receptora pasó a `try_join!`: con `join!`, si el emisor moría el receptor esperaba a que NTTTCP agotase su propio plazo; ahora el fallo de los mensajes suelta el motor y su `Drop` lo mata al instante. Verificado con 2 pruebas deterministas (canal que cae tras la primera pata con motores de laboratorio; cancelación ≠ pérdida) y con NTTTCP real: `t171_una_perdida_de_canal_en_la_segunda_pata_guarda_la_sesion_incompleta` cancela en B mientras B emite la vuelta —sin punto cooperativo, B aborta su tarea y A solo ve caer el canal— y comprueba que A termina `Failed`, guarda `incomplete`/`is_partial` con `forward_bps` y **sin** `reverse_bps`, B queda `Cancelled` sin guardar nada, y no queda `ntttcp.exe`. **No cubierto**: la reconexión con el mismo `sessionId` (FR-024 dice «MAY», es opcional) y el latido cada 1 s con «Reconectando…» de §8.5 **no existen**; el marcado de huecos en muestras de una pérdida temporal sobre un canal que sí se recupera no tiene camino porque no hay recuperación; y la pérdida en la **primera** pata sigue sin guardar nada (no hay ninguna dirección completa que conservar)
...
- [ ] T181 Pantalla de Svelte para el consentimiento de sesión entrante: escucha `session://incoming-request`, muestra el equipo y el plan, y llama a `session_incoming_respond` con la decisión; si al abrir la aplicación ya hay algo pendiente, `session_incoming_list` lo recupera. Backend ya cableado y probado (T177); esto es exclusivamente la mitad de interfaz, que necesita verse en un navegador o en la aplicación para darse por terminada, no solo compilar (missing) — **hecha el 2026-09-25, junto con el resto del cableado de la interfaz que faltaba (misma tarea de fondo que T150)**: `App.svelte` era un cascarón — «Inicio» mostraba una tarjeta estática con `peers.emptyTitle`/`peers.emptyDesc` y nunca invocaba `listPeers`; `PeersScreen.svelte`, `SessionScreen.svelte` y `ResultScreen.svelte` existían con sus pruebas de componente pero **ningún componente de `App.svelte` los montaba**, y `HistoryScreen.svelte` estaba escrita pero no enrutada. Ahora: (1) `features/peers/model.svelte.ts` (`PeersModel`) sondea `peers_list`+`peers_discovered_list` cada 2 s, funde cada equipo descubierto con su confianza guardada **solo si coinciden id + huella** (FR-011: un anuncio con huella distinta no hereda nada), calcula ocupado/incompatible a partir de `busy`/`protocolVersion`, y gestiona el diálogo de emparejamiento saliente (inicio, confirmación, cancelación, caducidad a los 60 s) — 9 pruebas. (2) `features/session/controller.svelte.ts` (`SessionController`) arranca `session_start`, sondea `session_get_state` cada 500 ms, traduce el estado `SCREAMING_SNAKE_CASE` de Rust a la fase de `SessionScreen`, calcula un progreso **estimado por tiempo transcurrido** (no del motor) y, al completar, **lee el resultado guardado** por el backend con reintentos de 300 ms (nunca fabrica uno) — 6 pruebas, con el fixture real de T160 como dato de la prueba. (3) `App.svelte` monta `PeersScreen`→`SessionScreen`→`ResultScreen`→`HistoryScreen` en el router existente. (4) **Consentimiento entrante (T181 propiamente)**: `lib/api/consent.ts` exporta los cuatro comandos y dos wrappers de evento (`session://incoming-request`, `peers://incoming-pairing`); `features/consent/model.svelte.ts` (`ConsentModel`) sondea cada 3 s además de escuchar los eventos (un evento perdido antes de que la ventana esté lista no deja la solicitud invisible) y antepone una solicitud de sesión a un emparejamiento; `ConsentDialog.svelte` se monta una vez en `App.svelte`, fuera de cualquier ruta, porque una solicitud entrante puede llegar en cualquier pantalla — 4 pruebas del modelo. **Un límite de arquitectura real que esto expuso**: `check-imports.mjs` prohíbe importar `@tauri-apps/api` fuera de `src/lib/api`; los dos controladores nuevos importaban `listen` directamente y hubo que pasar por wrappers (`onSampleBatch` ya existente, `onIncomingSessionRequest`/`onIncomingPairingRequest` nuevos). **Verificado**: `pnpm verify` completo (tipos, formato, lint, imports, Rust, 128 tests Vitest) y `cargo test` completo (173+ tests) en verde. **Verificado además con `pnpm exec vite dev` y Playwright real (2026-09-25, tras esta misma tarea)**: la extensión de Chrome no conectó en esta sesión, y los dos primeros intentos con Playwright agotaron el tiempo de espera al navegar a `localhost` — la causa era un servidor de una ejecución anterior que ya ocupaba el puerto; con el servidor realmente arriba, la navegación funcionó a la primera. «Inicio» renderiza el estado vacío «No se han detectado equipos» junto con el aviso de error de `peers_list`/`peers_discovered_list` (`NB-INTERNAL-001`, esperado sin backend Tauri) mostrado y cerrable; «Historial» renderiza sus filtros y «No hay pruebas registradas todavía». Ninguno de los dos rompe la interfaz al fallar sus llamadas nativas: cada error se registra por el logger y se refleja donde corresponde, tal como se diseñó. **Sigue NO VERIFICABLE**: el WebView2 real y un backend Tauri de verdad detrás (nada de esto ejercita `session_start`, el diálogo de emparejamiento saliente ni el de consentimiento entrante con datos reales). **No hecho**: pantallas de progreso/comprobaciones detalladas del preflight (`PreflightScreen` sigue sin montar), `AdvancedPlan` (el plan es siempre el estándar), reconexión tras pérdida de canal, y el estado «Ocupado»/«Versión incompatible» de mDNS nunca se ha visto porque no hay un segundo equipo real anunciándose
- [x] T182 Emparejamiento entrante (`PAIR_REQUEST`, FR-012) — **backend cerrado el 2026-09-25; sigue sin pantalla (T181)**: `control/despachador.rs` ya no descarta el mensaje: `control/pairing_incoming.rs` verifica el código y la huella **antes** de enseñar nada a la persona (un código que no cuadra recibe `verificationFailed` sin preguntar y sin distinguir por qué), registra el emparejamiento pendiente, avisa por el evento nuevo `peers://incoming-pairing`, espera hasta 55 s la decisión (menos que los 60 s que espera el iniciador, para que su plazo no venza justo antes) y, solo con un sí, guarda al equipo como `Trusted` con autoaceptación desactivada **antes** de contestar `accepted:true` (si guardar falla, no se afirma una aceptación que no ocurrió). Un segundo emparejamiento del mismo equipo mientras hay uno pendiente se rechaza al instante. Para poder esperar a una persona, `pairing_flow::atender_emparejamiento` (decisión síncrona) se partió en `preparar_respuesta` + `contestar_emparejamiento`, y el almacén de consentimiento pasó a ser genérico (`Decisiones<T>`, compartido con T177). Dos comandos IPC nuevos (`peers_pairing_incoming_list`/`_respond`). Verificado con 4 pruebas sobre TLS real entre dos nodos (`session_service_real.rs`): aceptar guarda confianza sin autoaceptación y el código que ve B es el mismo que ve A; rechazar no guarda nada; un código incorrecto se rechaza sin preguntar ni avisar a la interfaz; un segundo intento simultáneo se rechaza. **No cubierto**: la pantalla (T181); el vencimiento de 55 s sin decisión (no se prueba esperando 55 s reales); las direcciones del peer no se persisten (`history` las carga vacías), y la que se calcula para mostrar usa el puerto de control por defecto (7411) por **inferencia**, porque el `HELLO` no declara el puerto; y, como siempre, dos equipos físicos (T180)
