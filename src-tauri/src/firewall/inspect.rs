...
impl FirewallInspector {
    ...
    pub fn inspect_rule(rule_name: &str, expected_port_range: &str, expected_proto: &str) -> FirewallInspection {
    ...
    #[cfg(target_os = "windows")]
    fn inspect_windows(rule_name: &str, expected_port_range: &str, expected_proto: &str) -> FirewallInspection {
        ...
        match output {
            Ok(out) => {
                ...
                if !out.status.success() || stdout.contains("No rules match") || stdout.contains("No coincide ninguna regla") {
                ...
                let proto_match = stdout.to_lowercase().contains(&expected_proto.to_lowercase());
                let port_match = expected_port_range.is_empty() || stdout.contains(expected_port_range);
...
mod tests {
    ...
    fn test_inspect_nonexistent_rule() {
        let inspection = FirewallInspector::inspect_rule("ReglaInexistente_NetworkBench_XYZ_123", "5201", "TCP");
