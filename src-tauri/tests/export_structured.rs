use networkbench_lib::export::csv::{
    SampleCsvRow, UTF8_BOM, export_samples_csv, export_summary_csv, sanitize_csv_cell,
};
...
fn test_json_single_session_exact_units_and_schema() {
    ...
    let json_output = export_sessions_to_json(std::slice::from_ref(&session), false)
        .expect("Exportar JSON individual");
    ...
    let parsed: serde_json::Value =
        serde_json::from_str(&json_output).expect("Parsear JSON generado");
...
fn test_json_multiple_sessions_array_output() {
    ...
    let parsed: Vec<serde_json::Value> =
        serde_json::from_str(&json_output).expect("Parsear array JSON");
...
fn test_csv_bom_and_locale_separators_spanish_and_english() {
    ...
    let csv_es = export_summary_csv(std::slice::from_ref(&session), false, ';', ',')
        .expect("Exportar CSV español");

    assert!(
        csv_es.starts_with(UTF8_BOM),
        "El CSV debe comenzar con UTF-8 BOM"
    );
    let lines_es: Vec<&str> = csv_es.lines().collect();
    assert!(lines_es.len() >= 2);
    assert!(
        lines_es[0]
            .contains("sessionId;fecha;equipoLocal;equipoRemoto;direccion;protocolo;velocidadMbit")
    );
    ...
    let csv_en = export_summary_csv(std::slice::from_ref(&session), false, ',', '.')
        .expect("Exportar CSV inglés");

    assert!(
        csv_en.starts_with(UTF8_BOM),
        "El CSV debe comenzar con UTF-8 BOM"
    );
...
fn test_csv_formula_injection_sanitization() {
    ...
    assert_eq!(
        sanitize_csv_cell("@HYPERLINK('evil')"),
        "'@HYPERLINK('evil')"
    );
...
fn test_csv_samples_export() {
    ...
    assert_eq!(
        lines[0],
        format!("{UTF8_BOM}sessionId;direccion;equipo;tMs;rxMbit;txMbit;cpuPct")
    );
